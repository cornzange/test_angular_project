...
export class M1Component implements OnInit {
    ...
    items = []
    constructor(private task27Service: M1Service) {
    }
    addItem(title: string, completed: boolean) {
        console.log("title", title)
        console.log("completed", completed)
        this.task27Service.createToDo(title, completed)
    }
    // deleteItem(id: number) {
    //     this.task27Service.deleteToDo(id)
    // }
    // updateItem(id: number, completed: boolean) {
    //     this.task27Service.updateToDo(id, completed)
    // }
    ...
    getToDos() {
        this.items = this.task27Service.getAllToDo()
    }
}
